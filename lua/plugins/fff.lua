...
vim.keymap.set('n', '<leader><leader>', fff('find_files'), { desc = 'find files in project directory' })
vim.keymap.set('n', '<leader>/', fff('live_grep'), { desc = '(not fuzzy) find by grepping in project directory' })
vim.keymap.set('n', '<leader>fg', fff('live_grep', { grep = { modes = { 'fuzzy' } } }), {
    desc = 'fuzzy find by grepping in project directory',
})
vim.keymap.set({ 'n', 'x' }, '<leader>fw', fff('live_grep_under_cursor'), {
    desc = '[f]ind (grep) [v]isual selection or word under cursor',
})
vim.keymap.set('n', '<leader>fW', function()
    require('fff').live_grep({ query = vim.fn.expand('<cWORD>') })
end, { desc = '[f]ind current [W]ORD' })
vim.keymap.set('n', '<leader>fr', fff('resume'), { desc = '[f]ind in [r]esumed search' })
...
end, { desc = '[f]ind (grep) in [c]urrent buffer' })
vim.keymap.set('n', '<leader>fid', fff('find_files_in_dir', vim.fn.expand('~/projects/dotfiles')), {
